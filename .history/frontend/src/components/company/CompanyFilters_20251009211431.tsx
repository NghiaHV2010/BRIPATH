...
export default function CompanyFilters() {
  ...
  const { filterCompanies, fetchCompanies } = useCompanyStore();
  ...
  const handleInputChange = async (
    ...
    if (fieldName === "name") {
      setTimeout(async () => {
        await filterCompanies(
          1,
          currentFilters.name.trim() || undefined,
          currentFilters.location.trim() || undefined,
          currentFilters.field.trim() || undefined
        );
      }, 500);
    } else {
      await filterCompanies(
