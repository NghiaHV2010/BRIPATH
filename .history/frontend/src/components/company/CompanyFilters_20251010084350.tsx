...
import { fetchFields, fetchLocations } from "@/api/company_api"; // 👈 thêm dòng này
...
export default function CompanyFilters({
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        const [fieldData, locationData] = await Promise.all([
          fetchFields(),
          // fallback nếu backend chưa có API location → dùng mảng rỗng
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await filterCompanies(1, searchTerm.trim(), selectedLocation, selectedField, "");
  ...
  const handleLoadMore = async () => {
    ...
    if (nextVisible > displayedCompanies.length && lastFetchCount === 12) {
      ...
      setPage(nextPage);
      await filterCompanies(nextPage, searchTerm.trim(), selectedLocation, selectedField, "");
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 mb-8">
      <div className="text-center mb-6">
        ...
        <p className="text-slate-600">Nhập từ khóa, chọn lĩnh vực hoặc địa điểm để lọc kết quả</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto space-y-4">
        ...
        <div className="flex gap-3">
          ...
          >
            {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <SearchIcon />}
          </Button>
        </div>
