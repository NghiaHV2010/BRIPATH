...
export default function CompanyFilters({ onCompanyClick, onSearchStateChange }: CompanyFiltersProps = {}) {
  const [searchTerm, setSearchTerm] = useState("");
  const [isSearching, setIsSearching] = useState(false);

  const { filterCompanies, fetchCompanies, companies, isLoading } =
    useCompanyStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSearching(true);
    onSearchStateChange?.(true); // Thông báo CompaniesPage đang search
  ...
  const handleReset = async () => {
    ...
    onSearchStateChange?.(false); // Thông báo CompaniesPage không còn search
      ...
      {hasResults && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <p className="text-green-800 text-center">
            Tìm thấy {companies.length} công ty phù hợp với "{searchTerm}"
          </p>
        </div>
      )}
      ...
      {isSearching && isLoading && (
        <div className="mt-8">
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-slate-600">Đang tìm kiếm công ty...</p>
          </div>
        </div>
      )}

      {/* Search Results - Company List */}
      {isSearching && !isLoading && companies.length > 0 && (
        <div className="mt-8">
