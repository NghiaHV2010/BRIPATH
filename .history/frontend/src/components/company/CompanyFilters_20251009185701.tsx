...
interface CompanyFiltersProps {
  onFilter: (filters: { name?: string; location?: string; field?: string }) => void;
  onReset: () => void;
}

export default function CompanyFilters({ onFilter, onReset }: CompanyFiltersProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-slate-900">Filter Companies</h3>
        {hasActiveFilters && (
          <button
            type="button"
            onClick={handleReset}
