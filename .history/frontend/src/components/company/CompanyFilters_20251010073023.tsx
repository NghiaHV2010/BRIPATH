...
export default function CompanyFilters({ onCompanyClick }) {
  ...
  const handleLoadMore = async () => {
    ...
    // Nếu đã hiển thị hết 12 (hoặc hết danh sách), mới gọi backend để lấy page mới
    if (nextVisible > filteredCompanies.length && filteredCompanies.length % 12 === 0) {
      const nextPage = page + 1;
      setPage(nextPage);
      await filterCompanies(nextPage, searchTerm.trim(), "", "", "");
    }

    // Dù sao cũng tăng số hiển thị
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8 mb-8">
      ...
      {hasResults && (
        <div className="mt-8">
          ...
          {/* Grid công ty (2 cột, 3 hàng = 6 item) */}
