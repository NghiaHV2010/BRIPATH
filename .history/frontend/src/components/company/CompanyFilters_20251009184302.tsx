...
export function CompanyFilters({
  ...
  // Internal state for UI (with "all" values)
  ...
  const handleFilterChange = (
    key: keyof CompanyFilterValues,
    value: string
  ) => {
    // Update UI state
    setUiFilters((prev) => ({ ...prev, [key]: value === "all" ? "all" : value }));
  ...
  const handleClearFilters = () => {
    ...
    const emptyUiFilters = { name: "", location: "all", field: "all" };

    setFilters(emptyFilters);
    setUiFilters(emptyUiFilters);
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t">
              ...
              {filters.field && (
                <Badge variant="outline" className="flex items-center gap-1">
                  ...
                  Lĩnh vực:{" "}
                  {
                    fieldOptions.find((opt) => opt.value === filters.field)
                      ?.label
                  }
                  <X
                    className="h-3 w-3 cursor-pointer hover:text-red-500"
