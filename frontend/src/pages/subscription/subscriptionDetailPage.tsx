...
import {
  ...
  PaymentSuccess,
} from "../../components/subscription/paymentMethod";
...
const subscriptionPlans: SubscriptionPlan[] = [
  ...
  },
  {
    ...
    description:
      "Gói phổ biến với nhiều tính năng hữu ích cho việc định hướng nghề nghiệp",
    features: [
...
export default function SubscriptionDetailPage() {
  ...
  const config = tierConfig[plan.tier as keyof typeof tierConfig];
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="relative z-10 container mx-auto px-4 py-12">
          ...
          <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12">
            {/* Plan Details */}
            <div
              className={`${config.bgColor} rounded-3xl p-8 border-2 ${config.borderColor} shadow-xl`}
            >
              <div className="text-center mb-8">
                <div
                  className={`inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-r ${config.gradient} mb-4 shadow-lg`}
                >
                  <span className="text-3xl">{config.icon}</span>
                </div>

                <h1
                  className={`text-4xl font-bold bg-gradient-to-r ${config.gradient} bg-clip-text text-transparent mb-2`}
                >
                ...
                <ul className="space-y-3">
                  {plan.features.map((feature: string, index: number) => (
                    <li key={index} className="flex items-start space-x-3">
                      <div
                        className={`flex-shrink-0 w-6 h-6 rounded-full bg-gradient-to-r ${config.gradient} flex items-center justify-center mt-0.5`}
                      >
                        <svg
