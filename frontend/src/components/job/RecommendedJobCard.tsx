...
export const RecommendedJobsCard: React.FC<RecommendedJobsCardProps> = ({
  ...
  if (isLoading) {
    return (
      <Card className="bg-white shadow-lg rounded-2xl w-full">
  ...
  return (
    <Card className="bg-white shadow-lg rounded-2xl overflow-hidden w-full">
      <CardHeader className="bg-linear-to-r from-yellow-50 to-orange-50 pb-4">
        <CardTitle className="text-lg sm:text-xl font-bold text-slate-900 flex items-center gap-2">
          Việc làm đề xuất
        </CardTitle>
      ...
      <CardContent className="p-4 sm:p-6 space-y-3 sm:space-y-4">
        {jobs.map(job => (
          <div
            ...
            className="p-3 sm:p-4 rounded-xl hover:bg-linear-to-r hover:from-yellow-50 hover:to-orange-50 transition-all cursor-pointer group border border-transparent hover:border-yellow-200 hover:shadow-md"
          >
            {/* Job Header */}
            <div className="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
              {/* Company Logo */}
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-lg overflow-hidden border-2 border-slate-100 group-hover:border-yellow-200 transition-colors shrink-0">
                {job.avatar_url ? (
                  <img
                    src={job.avatar_url}
                    alt={job.username}
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-slate-900 group-hover:text-yellow-600 transition-colors text-xs sm:text-sm mb-1 line-clamp-2">
                  {job.job_title}
                </h3>
            ...
            <div className="space-y-1.5 sm:space-y-2">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-slate-600">
                <DollarSign className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0 text-green-600" />
            ...
            {job.label_name && (
              <div className="mt-2 sm:mt-3">
                <Badge
                  variant="outline"
                  className="bg-linear-to-r from-yellow-100 to-orange-100 text-orange-700 border-orange-200 text-[10px] sm:text-xs"
                >
                  <Star className="w-2.5 h-2.5 sm:w-3 sm:h-3 mr-1 fill-orange-500 text-orange-500" />
                  {job.label_name}
                </Badge>
              </div>
            )}
