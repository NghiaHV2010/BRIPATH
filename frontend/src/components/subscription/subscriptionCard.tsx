...
export function SubscriptionCard({ plan, billingCycle, onSelect }: SubscriptionCardProps) {
  ...
  return (
    ...
    >
      {/* Glow Effect */}
      <div 
        className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${config.borderGradient} opacity-0 transition-opacity duration-500 ${
      ...
      {(plan.isPopular || plan.isRecommended) && (
        <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-10">
          <Badge className={`${config.badgeColor} text-white px-4 py-1 text-sm font-semibold shadow-lg animate-pulse`}>
      ...
      {/* Main Card */}
      <div 
        className={`relative bg-white/90 backdrop-blur-sm rounded-2xl border-2 border-transparent p-6 lg:p-8 transition-all duration-300 ${config.shadowColor} ${isHovered ? `shadow-2xl ${config.glowColor}` : 'shadow-lg'} ${plan.tier === 'gold' ? 'ring-2 ring-yellow-400/40' : ''} h-full flex flex-col`}
        style={{
          background: `linear-gradient(white, white) padding-box, linear-gradient(135deg, ${
            plan.tier === 'gold' ? '#fbbf24, #f59e0b' : plan.tier === 'silver' ? '#94a3b8, #64748b' : '#f59e0b, #ea580c'
          }) border-box`,
        }}
      >
        {/* Header */}
        <div className="text-center mb-6">
          ...
          <h3 className={`text-2xl lg:text-3xl font-bold bg-gradient-to-r ${config.gradient} bg-clip-text text-transparent mb-2`}>
        ...
        <div className="flex-1">
          ...
          <div className="flex items-baseline justify-center mb-2">
            <span className={`text-4xl lg:text-5xl font-bold bg-gradient-to-r ${config.gradient} bg-clip-text text-transparent`}>
          ...
          {billingCycle === 'yearly' && (
            <p className="text-green-600 text-sm font-semibold mt-1">
              💰 Tiết kiệm {discount > 0 ? `${discount}%` : '2 tháng miễn phí'}
            </p>
          )}
          </div>
          ...
          <div className="mb-6">
            <ul className="space-y-3">
              {(plan.features.slice(0, 4)).map((feature, index) => (
                <li key={index} className="flex items-start space-x-3">
                  <div className={`flex-shrink-0 w-5 h-5 rounded-full bg-gradient-to-r ${config.gradient} flex items-center justify-center mt-0.5`}>
                    <CheckIcon />
                  </div>
            ...
            {plan.features.length > 4 && (
              <p className="mt-3 text-xs text-gray-500">… và nhiều lợi ích khác</p>
            )}
          </div>
        </div>
        ...
        <Button
          ...
          className={`w-full py-3 lg:py-4 text-base lg:text-lg font-semibold rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg ${
            plan.tier === 'gold' 
              ? 'bg-gradient-to-r from-yellow-500 via-amber-500 to-yellow-600 hover:from-yellow-600 hover:via-amber-600 hover:to-yellow-700 text-white shadow-yellow-500/25 hover:shadow-yellow-500/40' 
              : plan.tier === 'silver'
              ? 'bg-gradient-to-r from-slate-500 via-gray-500 to-slate-600 hover:from-slate-600 hover:via-gray-600 hover:to-slate-700 text-white shadow-slate-500/25 hover:shadow-slate-500/40'
              : 'bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-600 hover:via-orange-600 hover:to-amber-700 text-white shadow-amber-500/25 hover:shadow-amber-500/40'
          }`}
        >
          Xem chi tiết
          <SmallIcon />
        ...
        <div className="mt-3 text-center">
          {plan.tier === 'gold' && (
            <p className="text-xs text-yellow-700">Gói cao cấp với nhiều đặc quyền</p>
          )}
          {plan.tier === 'silver' && (
            <p className="text-xs text-slate-600">Gói phổ biến, cân bằng chi phí - lợi ích</p>
          )}
        </div>
      </div>
    </div>
