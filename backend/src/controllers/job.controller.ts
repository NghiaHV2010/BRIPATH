...
export const getJobByID = async (req: Request, res: Response, next: NextFunction) => {
    const { jobId, userId } = req.query as GetJobByIDRequestDto;
...
export const getJobsByFilter = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const { jobs, total_jobs } = await getJobsByFilterService(page, userId, filters);
...
export const getJobsByCompanyId = async (req: Request, res: Response, next: NextFunction) => {
    ...
    page -= 1;
    try {
        const { jobs, total_jobs } = await getJobsByCompanyIdService(page, company_id);
...
export const getJobDetailsByCompanyId = async (req: Request, res: Response, next: NextFunction) => {
    const { company_id } = req.user as AuthUserRequestDto;
    const { jobId } = req.params as GetJobByIDRequestDto;

    try {
        const job = await getJobDetailsByCompanyIdService(jobId, company_id);
...
export const createJob = async (req: Request, res: Response, next: NextFunction) => {
    const { id, username, company_id } = req.user as AuthUserRequestDto;
    const { id: subscription_id } = req.plan;

    const jobData = req.body as CreateJobRequestDto;
    ...
    const convert_startDate = new Date(jobData.start_date);
    ...
    if (jobData.end_date) {
        convert_endDate = new Date(jobData.end_date);

        if (isNaN(convert_endDate as unknown as number)) {
        ...
        const result = await createJobService(
            {
                ...
                end_date: jobData.end_date ? convert_endDate : null,
            },
            username,
            id,
            company_id,
...
export const updateJob = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const {
        ...
        end_date,
        category,
    } = req.body as CreateJobRequestDto;
